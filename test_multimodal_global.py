import requests
import base64
from google.auth import default
from google.auth.transport.requests import Request as AuthRequest
from pydantic_settings import BaseSettings, SettingsConfigDict
import os

class Settings(BaseSettings):
    PROJECT_ID: str = "jwlee-argolis-202104"
    LOCATION: str = "us-central1"
    model_config = SettingsConfigDict(
        env_file=os.path.abspath(os.path.join(os.path.dirname(__file__), ".env")), 
        env_file_encoding="utf-8"
    )


settings = Settings()

def get_token():
    credentials, _ = default()
    auth_request = AuthRequest()
    credentials.refresh(auth_request)
    return credentials.token

def generate_description_global(video_bytes, model_id="gemini-3.1-flash-lite-preview"):
    token = get_token()
    project_id = settings.PROJECT_ID
    
    # Global Endpoint doesn't use {location}- prepended to domain usually
    url = f"https://aiplatform.googleapis.com/v1/projects/{project_id}/locations/global/publishers/google/models/{model_id}:generateContent"
    
    headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
    
    # Just use short mock bytes for metadata test
    b64_data = base64.b64encode(video_bytes).decode('utf-8')
    payload = {
        "contents": [{
            "role": "user",
            "parts": [
                {"inlineData": {"mimeType": "video/mp4", "data": b64_data}},
                {"text": "Describe this video clip segment."}
            ]
        }],
        "generationConfig": {"temperature": 0.2, "maxOutputTokens": 300}
    }

    print(f"Testing URL: {url}")
    response = requests.post(url, headers=headers, json=payload, timeout=60)
    print(f"Status: {response.status_code}")
    print(f"Response: {response.text}")
    return response.status_code

# Read dummy segment test file if any exists
segments_dir = "app/static/segments"
if os.path.exists(segments_dir):
    v_dirs = [d for d in os.listdir(segments_dir) if os.path.isdir(os.path.join(segments_dir, d))]
    if v_dirs:
         test_file = os.path.join(segments_dir, v_dirs[0], "segment_001.mp4")
         if os.path.exists(test_file):
              print(f"--- 🧪 Testing Global REST on {test_file} ---")
              with open(test_file, "rb") as f:
                   b = f.read()[:1000] # short chunk to force API test
              generate_description_global(b)
else:
    print("No static segments found for testing.")
