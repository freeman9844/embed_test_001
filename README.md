...
### 3. ⚖️ 상호 순위 결합(RRF) 및 키워드 가중 보정 시스템
- **순위 병합(RRF)**: 비주얼 랭킹과 한국어 텍스트 설명 랭킹의 최적 비중을 도출하여 융합 순위를 산출합니다.
- **어휘별 직접 보정 (Word-Level Boost)**: 임베딩 보정이 놓치기 쉬운 명사 키워드(`'유튜브'`, `'깎는'`)에 대해, 묘사문 내 단어 일치 시 **절대 가중치 보너스 점수**를 가산해 순위를 극대화합니다.
